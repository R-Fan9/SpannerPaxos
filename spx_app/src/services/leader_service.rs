...
use crate::handles::ReplicateWriteResult;
use crate::managers::FollowerManager;
use crate::payloads::ReplicateWritePayload;
use chrono::{DateTime, Utc};
use spx_core::members::Leader;
use spx_lib::true_time::TrueTimeService;
use spx_lib::wal::WriteAheadLogService;
use std::error::Error;
use uuid::Uuid;

// A service that handles leader-related operations
...
impl LeaderService {
    async fn save_write(&mut self, entry: String) -> Result<(), Box<dyn Error>> {
        let write_time = self.get_latest_timestamp();
        let slot_number = self.leader.get_next_index(None);

        // Increment the leader's next index (slot number)
        ...
        // Start the Paxos write to replicate the current log entry to all the followers
    ...
    async fn process_replicate_write(
        &mut self,
        command: ReplicateWriteCommand,
        resp: ReplicateWriteResult,
    ) -> Result<(), Box<dyn Error>> {
        let resp = resp?.into_inner();
        if resp.term_number < self.leader.get_term_number() {
            // The response from the follower is for an older term, so ignore it
            return Ok(());
        }

        // Update the follower's match index to indicate that the log entry has been persisted to the follower's local WAL
        let follower_id = Uuid::parse_str(&resp.follower_id)?;
        let slot_number = resp.slot_number;
        self.leader
            .update_match_index(slot_number, Some(follower_id));

        if self.leader.has_committed(slot_number) {
            // The log entry at the given slot number has already been committed, return
            return Ok(());
        }

        if self.leader.has_quorum(slot_number) {
            // Quorum has reached for the log entry with the given slot number,
            // Signal that the log entry has been persisted by a quorum of Paxos group members
