...
To detect Leader failure and trigger a new election, every Follower runs an expiry-based countdown rather than waiting for an explicit step-down message. The Leader renews that countdown implicitly via the leader lease mechanism on every accept round-trip.
...
The Leader Lease, as introduced in Google Spanner, solves this with a time-bounded guarantee: a Leader's lease interval is guaranteed to expire **before** a quorum of Followers will agree to elect a new one. A Follower that promises to follow a Leader extends its local lease expiry forward by the lease duration. The Leader extends its own lease only when a quorum of Followers has confirmed receipt of an accept request (meaning their leases have been extended), and uses the **oldest** of those confirmed contact times as the lease base. This conservative anchor ensures the leader's lease always expires first, making it safe to serve reads without a quorum check.
